...
WAREHOUSE_BOX_HALF = 12.0
TASK_CIRCLE_RADIUS = 8.0
TASK_CIRCLE_SIDES = 18
TASK_CIRCLE_COLOR = (255, 140, 0, 220)
FOLLOW_ROUTE_COLOR = (0, 0, 255, 255)
OTHER_ROUTE_LAYER = 40
FOLLOW_ROUTE_LAYER = 99
ROUTE_MARKER_SQUARE_SIZE = 3.0

STALL_SPEED_THRESHOLD = 0.2
STALL_TICKS_THRESHOLD = 10
WAREHOUSE_PARK_POS = 5.0
...
CAPACITY_PENALTY_FACTOR = 2.0  

TIME_PENALTY_BASE = 1.0
TIME_PENALTY_URGENT_THRESHOLD = 300  
TIME_PENALTY_CRITICAL_THRESHOLD = 100  
...
ACTIVE_ROUTE_POLY: Dict[str, str] = {}           
ACTIVE_EDGE_POLYGONS: Dict[str, List[str]] = {}  
WAREHOUSE_POLY_ID = "warehouse_outline_poly"
WAREHOUSE_POI_ID = "WAREHOUSE"
TASK_TARGET_SIG: Dict[str, Tuple[str, ...]] = {} 
TASK_TARGET_IDS: Dict[str, List[str]] = {}       
...
TASK_ASSIGN_TIME: Dict[str, int] = {}    
TASK_DONE_TIME: Dict[str, int] = {}      
TASK_HISTORY: Dict[str, List[Dict]] = {} 

TASK_ALLOCATION_HISTORY: List[Dict] = []  
VEHICLE_UTILIZATION: Dict[str, List[Tuple[int, int]]] = {}  

AGENTS: Dict[str, "VehicleAgent"] = {}

VEHICLE_RESPAWN_COOLDOWN: Dict[str, int] = {}  
...
def calculate_priority_multiplier(task_weight: float, task_id: str) -> float:
    ...
    if task_weight >= 5.0:
        ...
        return NORMAL_PRIORITY_MULTIPLIER  
    else:
...
def calculate_time_window_penalty(task_id: str, base_score: float, current_time: float) -> float:
    ...
    travel_time_estimate = base_score / 10.0  
    ...
    def process(self, now: int, mesh: "MeshSimulator"):
        while self.inbox:
            ...
            if mt == "FORWARD_ANNOUNCEMENT":
                ...
                best = msg.get("best", {"bid": VERY_HIGH_BID, "holder": None})  
...
class MeshSimulator:
    ...
    def _backtrace(self, forward_msg: Dict, sim_time: int):
        ...
        best = forward_msg.get("best", {"bid": VERY_HIGH_BID, "holder": None})  
        ...
        if winner is None:
            ...
            best = {"bid": VERY_HIGH_BID, "holder": winner}  
            ...
            for tid, delivery_edge in ag.assigned_tasks:
                if tid == current_task_id:
                    ...
                    try:
                        ...
                        if route_idx >= len(current_route) - 2:  
